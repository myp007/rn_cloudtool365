 ...
 */
import {Services, Api, Components, Storage, StringUtils, Config} from 'react-native-blue-book';
const {Modal} = Components;


async function getService(...args) {
    return Api.getServiceByUrl(Config.get('TENCENT_API_PATH'), ...args);
}
...
Services.Function10000201 = async function (params) {
    ...
    let data = await getService('10000201', map);
    console.log(data);
...
Services.Function10000204 = async function (id) {
    ...
    let data = await getService('10000204', map, null, false);
    if (data.errorCode == 0) {
        return data;
    } else {
        Modal.showAlert(data.errorMsg);
